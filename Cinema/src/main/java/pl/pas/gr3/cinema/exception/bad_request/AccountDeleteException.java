package pl.pas.gr3.cinema.exception.bad_request;

import pl.pas.gr3.cinema.exception.general.BadRequestException;
import pl.pas.gr3.cinema.util.I18n;

public class AccountDeleteException extends BadRequestException {

    public AccountDeleteException() {
        super(I18n.ACCOUNT_DELETE);
    }

    public AccountDeleteException(Throwable cause) {
        super(I18n.ACCOUNT_DELETE, cause);
    }
}
